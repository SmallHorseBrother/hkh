...
// Helper to compress images to save storage
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      // If data is corrupted, clear it to allow app to work
  ...
  // Safe storage handler that handles QuotaExceededError
  const safeLocalStorageSet = (key: string, value: any, setter: (val: any) => void) => {
    try {
      ...
      if (e.name === 'QuotaExceededError' || e.code === 22) {
        // If quota exceeded and it's an array, try trimming oldest items
        if (Array.isArray(value) && value.length > 1) {
           console.warn("Storage quota exceeded, trimming old data...");
           // Remove last few items (assuming newest are at index 0)
           // Recursive retry with fewer items
  ...
  const manualSaveCritical = () => {
    if (currentScan) {
      processCriticalSamples(currentScan, true);
    }
  };
  ...
  const confirmSaveStaple = () => {
    ...
    safeLocalStorageSet('nutrilens_staples_v1', newStaples, setStaples);
  ...
  // ... (logStaple, finishSession, startCamera, stopCamera functions remain the same)
  ...
  const finishSession = () => {
    if (currentScan) {
      processCriticalSamples(currentScan, false);
    }
    reset();
  };
  ...
  const captureImage = async () => {
    ...
    // Scale down image to save storage space
    const maxWidth = 800;
    ...
    // Use lower quality (0.6) for better compression
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = async (ev) => {
        ...
        // Compress uploaded image before usage
        const compressedBase64 = await compressImage(rawBase64);
        prepareAnalysis(compressedBase64);
      };
  ...
  const nudgeItemWeight = (itemId: string, factor: number) => {
    ...
    // Nudge logic: +10% or -10% of current weight
    const newWeight = Math.round(item.estimatedWeightGrams * factor);
    updateItemWeight(itemId, Math.max(1, newWeight)); // Prevent 0 or negative
  };

  // Safe total calculation with optional chaining to prevent crashes on bad data
  const totals = useMemo(() => {
    ...
    return currentScan.items.reduce((acc, item) => {
      ...
      const n = (item.nutrients || {}) as any; // Fallback for missing nutrients
      return {
      ...
      <main className="pt-24 px-4 max-w-lg mx-auto">
        {state === AppState.IDLE && !isConfirming && (
          <div className="space-y-8 py-6 animate-in fade-in">
            {error && (
              <div className="bg-red-50 border border-red-100 text-red-500 px-4 py-3 rounded-2xl flex items-center gap-3 text-sm font-bold animate-in slide-in-from-top-2">
                <Icons.Warn />
                {error}
              </div>
            )}
          ...
          <div className="animate-in slide-in-from-bottom-10 space-y-6 pb-32">
            <div className="relative rounded-3xl overflow-hidden shadow-xl">
              ...
              <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full text-[10px] font-black text-white uppercase tracking-widest">
                {selectedModel === MODELS.PRO ? '精细模式' : '快速模式'}
              </div>
            </div>
        ...
        {state === AppState.ANALYZING && (
          <div className="fixed inset-0 bg-white z-50 flex flex-col items-center justify-center text-center p-8 space-y-6">
            ...
            <div className="space-y-2">
               ...
               <p className="text-green-600 text-[10px] font-black uppercase tracking-[0.2em] mb-4">正在通过 {selectedModel === MODELS.PRO ? '精细版模型' : '快速版模型'} 分析</p>
               
               <div className="bg-green-50/50 p-4 rounded-2xl max-w-[280px] mx-auto animate-in fade-in slide-in-from-bottom-2">
                 <p className="text-green-800 text-xs font-bold mb-1">💡 健康冷知识</p>
                 <p className="text-green-600 text-sm font-medium leading-relaxed">
                   {HEALTH_TIPS[currentHealthTip]}
                 </p>
               </div>
            </div>
          </div>
          ...
          <div className="animate-in slide-in-from-bottom-10 space-y-8 pb-32">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 pt-4">
                ...
                <div className="space-y-3">
                  {currentScan.items.map((item) => {
                    ...
                    // Safe access for nutrients to prevent crashes if missing
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1 pr-4">
                              ...
                              <div className="flex items-center gap-3 mt-2">
                                {/* 手动输入框 */}
                                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-gray-50 border border-gray-100 focus-within:border-green-300 focus-within:ring-2 focus-within:ring-green-100 transition-all">
                                  <input 
                                    type="number"
                                  ...
                                  />
                                  <span className="text-gray-400 font-medium text-xs">g</span>
                                </div>
                                ...
                                <span className="text-xs text-gray-400">摄入: <strong className="text-gray-700">{Math.round(item.estimatedWeightGrams * factor)}g</strong></span>
                              </div>
                            </div>
                            <div className="text-right flex flex-col items-end">
                              <span className="block text-green-600 font-black whitespace-nowrap">{Math.round((n.calories || 0) * factor)} kcal</span>
                          ...
                          <div className="px-5 pb-5 pt-0 animate-in fade-in slide-in-from-top-1">
                            <div className="border-t border-gray-50 pt-4 grid grid-cols-2 gap-2">
                              <div className="bg-gray-50/50 p-2 rounded-xl flex justify-between items-center">
                                ...
                                <span className="text-xs font-black">{((n.protein || 0) * factor).toFixed(1)}g</span>
                              </div>
                              <div className="bg-gray-50/50 p-2 rounded-xl flex justify-between items-center">
                                <span className="text-[10px] font-bold text-gray-400">碳水</span>
                                <span className="text-xs font-black">{((n.carbs || 0) * factor).toFixed(1)}g</span>
                              </div>
                              <div className="bg-gray-50/50 p-2 rounded-xl flex justify-between items-center">
                                <span className="text-[10px] font-bold text-gray-400">脂肪</span>
                                <span className="text-xs font-black">{((n.fat || 0) * factor).toFixed(1)}g</span>
                              </div>
                              <div className="bg-gray-50/50 p-2 rounded-xl flex justify-between items-center">
                                <span className="text-[10px] font-bold text-gray-400">纤维</span>
                                <span className="text-xs font-black">{((n.fiber || 0) * factor).toFixed(1)}g</span>
                              </div>
                            </div>
                          </div>
                        )}
              ...
              <div className="space-y-3 mt-4">
                <button onClick={finishSession} className="w-full bg-gray-900 text-white rounded-[1.5rem] py-5 font-black shadow-xl active:scale-95 transition-all text-lg">确认记录并完成</button>
                
                <div className="relative">
                   {manualMarkError && (
                      <div className="absolute bottom-full left-0 right-0 mb-2 text-center animate-in slide-in-from-bottom-2 fade-in">
                         <span className="inline-block bg-red-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-lg relative">
                            {manualMarkError}
                   ...
                   <button 
                     ...
                     className={`w-full py-3 text-xs font-bold flex items-center justify-center gap-1 transition-all active:scale-95 ${
                       hasSavedCritical 
                         ? 'text-green-500 bg-green-50 rounded-xl cursor-default' 
                         : 'text-red-400 hover:text-red-500 hover:bg-red-50 rounded-xl'
                     }`}
                   >
                      {hasSavedCritical ? (
                        <><Icons.Check /> 已标记为偏差样本</>
                      ) : (
                        <><Icons.Warn /> 认为 AI 估算偏差大？点击标记样本</>
                      )}
                   </button>
                </div>
              </div>
        ...
        {state === AppState.HISTORY && (
          <div className="animate-in slide-in-from-bottom-10 space-y-6 pb-20">
             ...
             {/* 偏差样本库入口 */}
             <button onClick={() => setState(AppState.CRITICAL_SAMPLES)} className="w-full bg-red-50 border border-red-100 rounded-[1.5rem] p-4 flex items-center justify-between shadow-sm active:scale-95 transition-transform">
                <div className="flex items-center gap-3">
                   <div className="p-2 bg-red-100 text-red-600 rounded-full"><Icons.Warn /></div>
                   <div className="text-left">
                      <p className="text-sm font-bold text-red-800">偏差样本库</p>
                      <p className="text-xs text-red-400">已归档 {criticalSamples.length} 个 AI 偏差案例</p>
                   </div>
                </div>
                <div className="text-red-300"><Icons.ChevronDown /></div>
             </button>
        ...
        {state === AppState.CRITICAL_SAMPLES && (
           <div className="animate-in slide-in-from-bottom-10 space-y-6 pb-20">
             <div className="flex items-center gap-4">
                <button onClick={() => setState(AppState.HISTORY)} className="p-3 bg-white rounded-full text-gray-400 shadow-sm"><Icons.ArrowLeft /></button>
                <h2 className="text-xl font-black text-gray-800">AI 偏差样本库</h2>
             </div>
             <p className="text-sm text-gray-500 px-1">当您对 AI 的估算结果修正超过 30% 时，系统会自动将该样本保存至此，以便后续分析。</p>

             <div className="space-y-4">
                {criticalSamples.length > 0 ? criticalSamples.map(sample => (
                   <div key={sample.id} className="bg-white rounded-[1.5rem] p-4 border border-red-100 shadow-sm space-y-3">
                      <div className="flex gap-4">
                         ...
                         <div className="flex-1 space-y-1">
                            ...
                            <div className="inline-block px-2 py-1 bg-red-100 text-red-600 rounded-lg text-[10px] font-bold mt-1">
                               偏差 {sample.deviationPercent > 0 ? '+' : ''}{sample.deviationPercent}%
                            </div>
                         </div>
                      </div>
                      <div className="bg-gray-50 rounded-xl p-3 flex justify-between items-center text-xs">
                         <div className="text-center">
                            <p className="text-gray-400 mb-1">AI 预估</p>
                            <p className="font-black text-gray-800 text-lg">{Math.round(sample.aiWeight)}g</p>
                         </div>
                         <div className="text-gray-300">➔</div>
                         <div className="text-center">
                            <p className="text-gray-400 mb-1">您修正为</p>
                            <p className="font-black text-green-600 text-lg">{Math.round(sample.userWeight)}g</p>
                         </div>
                      </div>
                   </div>
                )) : (
